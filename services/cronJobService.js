...
const { sendOneHourReminderEmail, sendOneDayReminderEmail } = require('../lib/emailService');
...
class CronJobService {
  ...
  formatTime(momentObj) {
    return {
      ...
      ist: momentObj.clone().tz('Asia/Kolkata').format('YYYY-MM-DD HH:mm:ss [IST]'),
  ...
  startCronJobs() {
    ...
    cron.schedule('* * * * *', () => {
      ...
      console.log('\n' + '='.repeat(80));
      console.log(`🕒 Cron Check Started at: ${timeInfo.utc} (${timeInfo.ist})`);
      ...
      console.log('='.repeat(80));
      console.log(`✅ Cron Check Completed at: ${moment().utc().format('YYYY-MM-DD HH:mm:ss [UTC]')}`);
      console.log('='.repeat(80) + '\n');
    });
  ...
  async checkPostEventFeedbackSchedules() {
    try {
      ...
      console.log(`   Search Window: ${windowStart.format('HH:mm:ss')} to ${windowEnd.format('HH:mm:ss')} UTC`);
      ...
      console.log(`   📊 Query Result: ${eventsWithFeedback.length} events found with feedback scheduled`);
      ...
      if (eventsWithFeedback.length > 0) {
        ...
        eventsWithFeedback.forEach((event, index) => {
          ...
          event.postEventFeedbackDetails.forEach((detail, detailIndex) => {
            ...
            console.log(`        • Scheduled: ${timeInfo.utc} (${timeInfo.ist})`);
            console.log(`        • Time Diff: ${timeDiff} minute(s)`);
            console.log(`        • Content: "${detail.bodyContent.substring(0, 50)}..."`);
            console.log(`        • Will Send: ${timeDiff <= 1 ? '✅ YES' : '❌ NO'}`);
          });
        });
      } else {
  ...
  async checkEventReminders() {
    try {
      ...
      console.log(`   📊 Query Result: ${eventsWithReminders.length} future events with reminders enabled`);
      ...
      for (const event of eventsWithReminders) {
        ...
        console.log(`     • Event Start: ${eventTimeInfo.utc} (${eventTimeInfo.ist})`);
        console.log(`     • 1-Hour Reminder: ${oneHourBeforeInfo.utc} (Diff: ${oneHourDiff}min) ${oneHourDiff <= 1 ? '✅ SEND NOW' : '⏭️ Skip'}`);
        console.log(`     • 1-Day Reminder: ${oneDayBeforeInfo.utc} (Diff: ${oneDayDiff}min) ${oneDayDiff <= 1 ? '✅ SEND NOW' : '⏭️ Skip'}`);
  ...
  async sendEventRemindersToAllAttendees(event, reminderType) {
    try {
      ...
      if (bookings.length === 0) {
        console.log(`     ⚠️  No confirmed bookings found for event: ${event.title}`);
        return;
      }
      ...
      bookings.forEach((booking, index) => {
        ...
        console.log(`       ${index + 1}. ${name || 'Unknown'} (${email || 'No Email'}) - ${booking.paymentStatus}`);
      });
      ...
      for (const booking of bookings) {
        try {
          ...
          if (userEmail) {
            ...
            successCount++;
            console.log(`       ✅ Sent to: ${userEmail} (${userName || 'Unknown'})`);
          } else {
          ...
          errorCount++;
          console.error(`       ❌ Failed for booking ${booking._id}:`, emailError.message);
        }
      }
      ...
      console.log(`       • Completed: ${endTimeInfo.utc} (${endTimeInfo.ist})`);
      
    } catch (error) {
      console.error(`❌ Error sending ${reminderType} reminders for event ${event.title}:`, error);
    }
  }
  ...
  async sendPostEventFeedbackToAllAttendees(event, feedbackDetail) {
    try {
      ...
      console.error(`❌ Error sending post-event feedback for event ${event.title}:`, error);
    }
  }
  ...
  async triggerEventReminders(eventId, reminderType = 'both') {
    try {
      ...
      console.log(`   Event Start: ${moment(event.startDateTime).utc().format('YYYY-MM-DD HH:mm:ss [UTC]')}`);
      ...
      return { success: true, message: `Event reminders (${reminderType}) triggered successfully` };
    } catch (error) {
  ...
  async triggerPostEventFeedback(eventId) {
    try {
      ...
      console.log(`   Event Found: ${event.title}`);
      console.log(`   Feedback Details: ${event.postEventFeedbackDetails.length} scheduled`);

      for (const feedbackDetail of event.postEventFeedbackDetails) {
        const scheduledTime = this.formatTime(moment(feedbackDetail.scheduledDateTime).utc());
        console.log(`   Scheduled Time: ${scheduledTime.utc} (${scheduledTime.ist})`);
        await this.sendPostEventFeedbackToAllAttendees(event, feedbackDetail);
      }

      return { success: true, message: 'Post-event feedback triggered successfully' };
    } catch (error) {
